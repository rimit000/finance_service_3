...
#  인증키와 API 설정
AUTH_KEY = quote("c60197ee6c4a9c3c08b206921f98ba56")  # 인증키는 quote로 인코딩
...
#  2. 전체 적금 상품 수집
def collect_saving_products(auth_key, group_code):
    all_products = []
    max_page = get_max_page(auth_key, group_code)

    for page in tqdm(range(1, max_page + 1), desc=f"[{group_code}] 적금 전체 페이지 수집 중"):
        params = {
            "auth": auth_key,
            "topFinGrpNo": group_code,
            "pageNo": page
        ...
        for base in base_list:
            ...
            for opt in matched_options:
                product = base.copy()
                product.update(opt)
                product["그룹코드"] = group_code  #  그룹코드 컬럼 추가
...
if __name__ == "__main__":
    ...
    for code in GROUP_CODES:
        df = collect_saving_products(AUTH_KEY, code)
        all_data = pd.concat([all_data, df], ignore_index=True)

    all_data.to_csv("적금_상품정보_API.csv", index=False, encoding="utf-8-sig")
    print(f" 저장 완료! 총 {len(all_data)}개 적금 상품 수집됨.")
